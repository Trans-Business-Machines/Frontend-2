...
=======
  // Filter logic (very basic, for demonstration)
  const filteredVisitors = sampleVisitors.filter(v => {
    let pass = true;
    // Period filter (mock logic, real logic should consider timeIn)
    if (filters.period && filters.period !== "All") {
      // Example: Treat before 12 as Morning, 12-5 as Afternoon, after 5 as Evening
      const hour = parseInt(v.timeIn.split(":")[0], 10);
      if (filters.period === "Morning") pass = pass && hour < 12;
      if (filters.period === "Afternoon") pass = pass && hour >= 12 && hour < 17;
      if (filters.period === "Evening") pass = pass && hour >= 17;
    }
    if (filters.date) pass = pass && v.date === filters.date;
    if (filters.host) pass = pass && v.host.toLowerCase().includes(filters.host.toLowerCase());
    if (filters.purpose) pass = pass && v.purpose.toLowerCase().includes(filters.purpose.toLowerCase());
    return pass;
  });

  return (
    <div className="main-app">
      <Sidebar active="visitorslog" />
          ...
          >
            {periods.map(option => (
              <option key={option.value} value={option.value}>
                {option.value === "All" ? "Time: All" : `Time: ${option.label}`}
              </option>
            ))}
          </select>
          <input
