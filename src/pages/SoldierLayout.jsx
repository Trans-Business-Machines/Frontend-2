...
export default function SoldierLayout() {
  ...
  // the menu array
  const menu = [
    { label: "Dashboard", icon: <span role="img" aria-label="dashboard"><AiOutlineDashboard /></span>, path: "/" },
    { label: "Check In", icon: <span role="img" aria-label="check-in"><LuMapPinCheckInside /></span>, path: "/check-in" },
    { label: "Check Out", icon: <span role="img" aria-label="check-out"><MdOutlineShoppingCartCheckout /></span>, path: "/check-out" },
    { label: "Visitors Log", icon: <span role="img" aria-label="visitors-log"><LuFolderSearch /></span>, path: "/visitors-log" },
  ];

  // Get user's actual name, fallback to "Soldier User"
  const soldierName = user?.name || user?.fullName || user?.displayName || "Soldier User";
  ...
  return (
    <div className="soldier-root-layout">
      <aside className="soldier-sidebar">
        ...
        >
          <span role="img" aria-label="logout" style={{ fontSize: 20, marginRight: 8 }}><HiOutlineLogout /></span>
          Log Out
        </button>
      </aside>
      <main className="soldier-main-panel">
        <header className="soldier-header">
          ...
          <div
            ...
            title="View Profile"
          >
            <span role="img" aria-label="soldier" style={{ fontSize: 24, marginRight: 10 }}><CgProfile /></span>
            {soldierName}
          </div>
        </header>
