...
export default function AdminProfile() {
  const { user } = useAuth() // fetch the logged-in user's ID
  const navigate = useNavigate()

  const { data: profile, isLoading } = useSWR(user?.userId ? `/users/${user.userId}` : null, getProfile)
      ...
      {isLoading ? (
        ...
        <div className="profile-container">
          <div className="profile-card">
              ...
              <div className="profile-row">
                <span className="profile-icon">
                  <svg width="18" height="18" fill="none">
                    <rect x="2.5" y="5.5" width="13" height="7" rx="1" stroke="#888" strokeWidth="1.5" />
                    <path d="M6 8h6M6 10h4" stroke="#888" strokeWidth="1.2" strokeLinecap="round" />
                  </svg>
                </span>
            ...
            <button className="profile-change-btn" onClick={() => navigate("/admin/profile/change-password")}>
              Change Password
            </button>
          </div>
        </div>
